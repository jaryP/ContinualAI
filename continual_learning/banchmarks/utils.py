...
from continual_learning.banchmarks.base import DatasetSplits
...
# class DatasetConcatenation(ConcatDataset):
#     def __init__(self, datasets: Iterable[Sequence],
...
class ConcatDataset:
    ...
    def __init__(self, datasets: Sequence[Union[UnsupervisedDataset, SupervisedDataset]]) -> None:
        super(ConcatDataset, self).__init__()

        assert len(datasets) > 0, 'datasets should not be an empty iterable'
        lens = len(datasets[0][0])
        assert all([len(d[0]) == lens for d in datasets]), 'The dataset __getitem__ are not comparable, because' \
